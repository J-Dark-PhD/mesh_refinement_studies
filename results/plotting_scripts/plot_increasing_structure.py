...
plt.vlines(max_size_range[-5], -1, 1, color="red", linestyles="dashed")
max_errors = [
    inventory_lipb_normalised[-5],
    inventory_structure_normalised[-5],
    inventory_baffle_normalised[-5],
    inventory_tungsten_normalised[-5],
    inventory_pipes_normalised[-5],
]
print("{:.2e}".format(max_size_range[-5]))
print("mesh size = {}".format(mesh_sizes[-5]))


leg = plt.legend(loc="upper left", bbox_to_anchor=(1.02, 1.05))
plt.ylim(-1, 1)
plt.xlabel("Maxiumum cell size (m)")
plt.ylabel("Relative inventory to reference (%)")
plt.title("Structure increasing")
plt.tight_layout()
